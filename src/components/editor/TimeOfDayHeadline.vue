<template>
  <div>
    <b-list-item class="time-of-day-headline">
      <!-- eslint-disable -->
      <input
        v-if="allowEdit"
        class="time input"
...
export default {
  ...
  methods: {
    ...
    ...mapActions('editor', ['setTimeOfDayStart']),
    updateHour(e) {
      this.inputHour = e.target.value
      this.setTimeOfDayStart({
        hour: this.inputHour,
        minute: this.timeOfDayStartMinute,
      })
    },
    updateMinute(e) {
      this.inputMinute = e.target.value
      this.setTimeOfDayStart({
        hour: this.timeOfDayStartHour,
