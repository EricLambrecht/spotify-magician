...
export default {
  ...
  playlistItems: (state, getters) =>
    getters.playlist.tracks.items.map((item, i) => ({
      ...item,
      _uniqueId: `${i}-${item.track.id}`,
    })),
  playlistUris: (state, getters) =>
    getters.playlistItems.map(item => item.track.uri),
  playlistIds: (state, getters) =>
    getters.playlistItems.map(item => item.track.id),
  playlistTrackNamesById: (state, getters) =>
    getters.playlist.tracks.items.reduce((acc, item) => {
      acc[item.track.id] = item.track.name
      return acc
    }, {}),
  playlistTrackCount: (state, getters) => getters.playlist.tracks.total,
  playlistLengthMs: (state, getters) =>
    getters.playlist.tracks.items.reduce(
